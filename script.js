...
// MODIFIED MARKSHEET VERSION
// ============================================================


// ============================================================
// STUDENT DATA
// 51 STUDENTS - ORIGINAL DATA PRESERVED
// ============================================================
...
const toppersGrid =
    document.getElementById('toppersGrid');
...
const resultSearchBtn =
    document.getElementById('resultSearchBtn');

const resultSuggestions =
    document.getElementById('resultSuggestions');
        ...
        function () {
            ...
            if (query.length === 0) {
                ...
                if (resultSuggestions) {
                    resultSuggestions.classList.remove(
                        'active'
                    );
                }
                ...
                showResultPlaceholder(
                    ...
                    'Enter name or enrollment number to view results'
                );
...
if (resultSearchBtn) {
    ...
    resultSearchBtn.addEventListener(
        'click',
        function () {

            const query =
                resultSearchInput
                    ? resultSearchInput.value.trim().toLowerCase()
                    : '';
...
function showResultSuggestions(results, query) {
    ...
    if (results.length === 0) {
        ...
        showResultPlaceholder(
            'No Student Found',
            'Please check the name or enrollment number'
        );
        ...
        results
            ...
            .map(student => `
                ...
                >
                    ...
                    <span class="s-percentage">
                        ${student.combined.percentage}%
                    </span>
    ...
    resultSuggestions.classList.add(
        'active'
    );
    ...
    document
        .querySelectorAll(
            '.result-search-suggestions .suggestion-item'
        )
        .forEach(item => {
...
function performResultSearch(query) {
    ...
    if (results.length === 0) {
        ...
        showResultPlaceholder(
            'No Student Found',
            'Please check the name or enrollment number and try again'
        );
...
// ============================================================
// MULTIPLE STUDENT RESULT SELECTION
// ============================================================
...
function displayMultipleResults(results) {
    ...
    resultContainer.innerHTML = `
        ...
        <div class="multiple-results">
            ...
            <div class="multiple-results-heading">
                ...
                <p>
                    Please select a student to view the marksheet.
                </p>

            </div>


            <div class="multiple-results-grid">

                ${results.map(student => `

                    <button
                        type="button"
                        class="student-result-option"
                        onclick="selectStudentResult('${String(student.id).replace(/'/g, "\\'")}')"
                    >
...
function selectStudentResult(id) {
...
function displayResult(student) {
    ...
    /*
     * HTML render hone ke baad QR generate.
     */
    setTimeout(() => {
...
function getMarksStatusClass(status) {
    ...
    if (value.includes('FAIL')) {
        ...
        return 'marksheet-status-fail';

    }


    if (value.includes('GRACE')) {

        return 'marksheet-status-grace';

    }


    return 'marksheet-status-pass';
...
function createSubjectRows(subjects = {}) {
    ...
    return `
        ...
        <div class="academic-term">
            ...
            <div class="term-total-row">
                ...
                <div class="term-total-spacer"></div>
...
function createResultCard(student) {
    ...
    const qrContainerId = `qrCodeContainer-${safeStudentId}`;
    const finalStatusClass = getMarksStatusClass(student.combined.finalStatus);

    return `
        <article class="result-card marksheet-card official-marksheet fade-in">

            <!-- OFFICIAL GSP LOGO -->
            <header class="marksheet-header">
                <img
                    class="marksheet-logo"
                    src="images/gsp-logo.png"
                    alt="Sant Shiromani Ravidas Global Skills Park Logo"
                >

                <h1>
                    Sant Shiromani Ravidas Global Skills Park (SSRGSP)
                </h1>

                <div class="marksheet-location">
                    Bhopal, Madhya Pradesh
                </div>
            </header>

            <!-- STUDENT SUMMARY -->
            <section class="marksheet-student-summary">
            ...
            <!-- Father's Name intentionally removed -->
            <section class="marksheet-section">
                <h2 class="marksheet-section-title">
                    Personal Information :
                </h2>
            ...
            <section class="marksheet-section">
                ...
                <div class="academic-records">
                    ...
                    <!-- TERM I -->
                    ${createTermMarkup('I', student.semester1)}

                    <!-- TERM II -->
                    ${createTermMarkup('II', student.semester2)}

                    <!-- GRAND TOTAL -->
                    <div class="grand-total-row">
                        <span>Grand Total Marks :</span>
                        <strong>${student.combined.total}</strong>
                    </div>
            ...
            <!-- FINAL RESULT -->
            <section class="marksheet-final-result">
                <span>Final Result :</span>

                <strong class="${finalStatusClass}">
...
function generateQRCode(studentId) {
    ...
    const container =
        document.getElementById(
            `qrCodeContainer-${safeStudentId}`
        );

    if (!container) {
        console.warn(
            'QR container not found for student:',
            studentId
        );
        return;
    }
    ...
    /*
        IMPORTANT:
        Every student's QR gets a different URL.

        Example:

        Student A:
        https://siyaramahirwar.github.io/ICTRESULT2026/index.html?id=SSRGSP%2FNET%2F2025%2F1%2F001

        Student B:
        https://siyaramahirwar.github.io/ICTRESULT2026/index.html?id=SSRGSP%2FNET%2F2025%2F1%2F002
    */

    const baseUrl =
        'https://siyaramahirwar.github.io/ICTRESULT2026/index.html';

    const qrUrl =
        `${baseUrl}?id=${encodeURIComponent(studentId)}`;
    ...
    toppersGrid.innerHTML =
        toppers.map(
            (student, index) => {
                ...
                return `
                    <div
                        class="topper-card ${rankClasses[index]} fade-in"
                        onclick="performResultSearch('${student.id}')"
                    ...
                    >
                        ...
                        <div class="topper-enroll">
                            ${student.id}
                        </div>

                        <div class="topper-percentage">
                            ${student.combined.percentage}%
                        </div>
...
function init() {
    ...
    /*
        Load common page sections first.
        Then automatically load marksheet
        if ?id=... exists in URL.
    */
...
console.log(
    '%c🎓 SSR-GSP Result Portal Loaded!',
    'font-size: 20px; font-weight: bold; color: #6c63ff;'
);

console.log(
    `%c📊 Total Students: ${studentData.length}`,
    'font-size: 14px; color: #4a4a6a;'
);

console.log(
    '%c🔍 Search: Type name or enrollment number in Result Section',
    'font-size: 12px; color: #888;'
);

console.log(
    '%c⌨️ Shortcut: Ctrl+K to focus search',
    'font-size: 12px; color: #888;'
);
    ...
    startBtn.addEventListener(
        ...
        async () => {
            ...
            try {
                ...
                scannerPlaceholder.classList.add(
                    'hidden'
                );
                ...
                updateStatus(
                    'Scanning QR Code... Hold certificate steady',
                    'loading'
                );
...
function handleQRCodeData(data) {
    ...
    if (match) {
        ...
        updateStatus(
            '❌ Invalid QR Code. Please scan a valid certificate QR code.',
            'error'
        );
...
function stopScanner() {
    ...
    if (scannerPlaceholder) {
        ...
        scannerPlaceholder.classList.remove(
            'hidden'
        );
    ...
    updateStatus(
        'Scanner stopped. Ready to scan again.',
        'info'
    );
}
...
console.log(
    '%c📷 QR Scanner Ready!',
    'font-size: 16px; font-weight: bold; color: #4285f4;'
);

console.log(
    '%c📌 QR Code will redirect to the URL found in the QR',
    'font-size: 12px; color: #888;'
);

console.log(
    '%c⌨️ Shortcut: Ctrl+Q to start/stop scanner',
    'font-size: 12px; color: #888;'
);
...
function printResult() {
    ...
    if (!marksheet) {
        ...
        alert(
            'Please search for a student first to print marksheet!'
        );
...
function downloadPDF() {
    ...
    if (!marksheet) {
        ...
        alert(
            'Please search for a student first to save PDF!'
        );
...
function shareResult() {
    ...
    if (!marksheet) {
        ...
        alert(
            'Please search for a student first to share their result!'
        );
        ...
        navigator.clipboard
            ...
            .then(
                () => {
                    ...
                    showToast(
                        'Result copied to clipboard! Share it anywhere.'
                    );
